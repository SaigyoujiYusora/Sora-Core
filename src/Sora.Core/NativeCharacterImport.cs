...
{
    public static DatabaseDocument Import(GameResources resources, string query)
    {
        ...
        var candidates = resources.Manifest.Assets.Where(x => x.Path.EndsWith("_uimodel.prefab", StringComparison.OrdinalIgnoreCase) && x.Path.Contains(query, StringComparison.OrdinalIgnoreCase))
            .DistinctBy(x => (x.Path, x.Bundle)).ToArray();
        Validation.Require(candidates.Length == 1, "Character selection is absent or ambiguous");
        var prefab = candidates[0];
